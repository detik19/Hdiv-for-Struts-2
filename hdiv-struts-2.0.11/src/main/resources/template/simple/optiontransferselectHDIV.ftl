...
<#include "/${parameters.templateDir}/simple/selectOptionTransferHDIV.ftl" />
...
<select 
	...
	<#assign merge = dataComposer.mergeParameters(parameters.name?default("")?html, parameters.doubleName?default("")?html) /><#t/>
...
>
	...
	<@s.iterator value="parameters.doubleList"><#t/>
        ...
        <#assign hdivValue = dataComposer.compose(parameters.name?default("")?html, "${doubleItemKeyStr?html}", false)/>
       	<#assign hdivValue = dataComposer.compose(parameters.doubleName?default("")?html, "${doubleItemKeyStr?html}", false)/>	
