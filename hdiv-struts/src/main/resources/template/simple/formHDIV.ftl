...
<#if parameters.action?exists>
  <#assign index = parameters.action.indexOf(";")>
  <#if (index > 0)>
	<#assign hdivValue = dataComposer.beginRequest(parameters.action.substring(0, index))/>
  <#else>
    <#assign hdivValue = dataComposer.beginRequest(parameters.action)/>
  </#if>
