<#include "/${parameters.templateDir}/simple/selectHDIV.ftl" />
<#assign startCount = 0/>
...
<select<#rt/>
 ...
 <#assign hdivValue = dataComposer.compose(parameters.doubleName?default("")?html, "", false)/><#rt/>
...
<@s.iterator value="parameters.list">
    ...
    <@s.iterator value="${parameters.doubleList}">
		...
		<#assign hdivValue = dataComposer.compose(parameters.doubleName?default("")?html, doubleItemValue, false)/><#rt/>        
    ${parameters.id}Group[${itemCount}][${doubleItemCount}] = new Option("${doubleItemValue}", "${hdivValue}");
    ...
    function ${parameters.id}Redirect(x) {
        ...
        for (i = 0; i < ${parameters.id}Group[x].length; i++) {
            ...
            <#if parameters.doubleNameValue?exists>
            	if (${parameters.id}Temp.options[i].text == '${parameters.doubleNameValue}') {            	
